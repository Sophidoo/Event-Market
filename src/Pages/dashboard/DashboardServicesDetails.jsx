...
const DashboardServicesDetails = () => {
  const { id } = useParams();
    ...
    <>
      <div className="dashboardDetailsHeader">
        <div className="overviewHeader">
          <h1>
            ...
            <span className="text-gray-600">Services</span> <ChevronRightIcon className="w-5 h-5 inline" />{" "}
            <span className="text-gray-600">Details</span>
          </h1>
          <div className="overviewHeaderProfile">
            <img src={service.vendor?.profile || "/placeholder.png"} alt="Vendor" className="object-cover" />
            <div className="overviewHeaderProfileDetails">
              <h4>{service.vendor?.companyName || "Vendor Name"}</h4>
      ...
      <section className="dashboardTopRentalDetails">
        <div className="leftRentalDetails">
          ...
          <img src={image || "/placeholder.png"} alt="Main service" className="bigImage object-cover" />
        </div>

        <div className="rightRentalDetails">
          <div className="rightRentalDetailsHeading">
            <small className="text-gray-600">{service.categoryType || "Unknown"}</small>
            <h1>{service.title}</h1>
            <div className="rightRentalRatingSystem">
              {[...Array(5)].map((_, i) => (
                <StarIcon
                  key={i}
                  className={`w-5 h-5 ${i < Math.round(service.avgRating || 0) ? "text-yellow-600" : "text-gray-300"}`}
                />
              ))}
          ...
          <h3 className="text-[#0B5850] font-medium">
            <small className="text-gray-600 font-medium">From:</small> ₦{service.minPrice?.toLocaleString() || "N/A"}
          </h3>
          ...
          <div className="detailsHolder">
            ...
            <ul className="text-gray-700">
              {service.offers?.length > 0 ? (
                service.offers.map((offering, index) => <li key={index}>{offering}</li>)
              ) : (
          ...
          <div className="detailsHolder">
            ...
            <ul className="text-gray-700">
              {service.prices?.length > 0 ? (
                ...
                <li>No pricing details provided.</li>
              )}
            </ul>
          </div>
          <div className="detailsHolder">
            ...
            <p className="text-gray-700">
              <span>Experience: {service.experience || "Not specified"} </span> <br />
              <span>Career highlight: {service.careerHighlight || "Not specified"} </span> <br />
              <span>Education and training: {service.education || "Not specified"} </span>
            </p>
          </div>
          <div className="detailsHolder">
          ...
          <div className="detailsHolder">
            ...
            <p className="text-gray-700">
              <span>Name: {service.vendor?.companyName || "Not specified"} </span> <br />
              <span>Location: {service.vendor?.location || "Not specified"} </span>
            </p>
          </div>
        </div>
      </section>
...
export default DashboardServicesDetails;