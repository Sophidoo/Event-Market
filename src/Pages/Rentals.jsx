...
const Rentals = () => {
  ...
  const fetchRentals = async () => {
    ...
    try {
      ...
      const [response, category] = await Promise.all([
        api.get(`/item/rentals/${current_page}/${per_page}`, {
          params: {
  ...
  const handleAddToWishlist = async (itemId, e) => {
  ...
  const renderPaginationButtons = () => {
    ...
    // First first page and ellipsis if needed
    if (startPage > 1) {
      buttons.push(
        <button
          key={1}
    ...
    <>
      <form className="rentalsHeroForm bg-white border-[1px] border-gray-200" onSubmit={handleSearch}>
        ...
        <div className="inputWrapper border-r-gray-200">
          ...
          <input type="date" name="end_date" id="end_date" value={searchDetails.end_date} onChange={handleChange} />
        </div>
          ...
          rentals.map((el) => (
            <div key={el.id} className="rentalCard" onClick={() => navigate(`/rental/details/${el.id}`)}>
              ...
              {wishlistStatus[el.id]?.isLoading ? (
                ...
                <HeartIcon
                  className={`saveIcon cursor-pointer ${wishlistStatus[el.id]?.isInWishlist ? "text-red-600" : "text-black"} hover:text-red-600`}
